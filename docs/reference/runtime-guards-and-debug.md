...
The summary reports the active quality profile, FPS, frame time, draw calls, triangles, active and total scene objects, budget breaches and bounded simulation collection counts.
...
`ANNALS_DEBUG` exposes subsystem summaries and acceptance hooks used by the repository's `.github/scripts/*.mjs` suites. These interfaces are test and diagnostic contracts rather than end-user controls.
