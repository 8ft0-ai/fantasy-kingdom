...
`scripts/build.py` is the boundary between those two models. It validates `src/manifest.txt`, assembles styles and scripts in the required order, and emits diagnostic module banners into the runtime. The generated structure is a traceability mechanism for a deliberately simple deployment artefact.
